...
SDK_VERSION=$(echo ${SDK_NAME} | grep -o '[\d.]+$')
...
xcodebuild -configuration "${CONFIGURATION}" -target "${TARGET_NAME}" -sdk "${OTHER_SDK_TO_BUILD}" ${ACTION} RUN_CLANG_STATIC_ANALYZER=NO ONLY_ACTIVE_ARCH=NO BITCODE_GENERATION_MODE=bitcode CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY= OBJROOT="${OBJROOT}/${TARGET_NAME}" BUILD_DIR="${BUILD_DIR}" BUILD_ROOT="${BUILD_ROOT}" CURRENT_PROJECT_VERSION="${CURRENT_PROJECT_VERSION}" CBL_VERSION_STRING="${CBL_VERSION_STRING}" CBL_SOURCE_REVISION="${CBL_SOURCE_REVISION}"  || exit 1
...
CURRENTCONFIG_DEVICE_DIR=${BUILD_DIR}/${CONFIGURATION}-${DEVICE}os
CURRENTCONFIG_SIMULATOR_DIR=${BUILD_DIR}/${CONFIGURATION}-${DEVICE}simulator
...
if [ -d "$CURRENTCONFIG_DEVICE_DIR" ]; then
	if [ -d "$CURRENTCONFIG_SIMULATOR_DIR" ]; then
		lipo -create -output "${CREATING_UNIVERSAL_DIR}/${EXECUTABLE_NAME}" "${CURRENTCONFIG_DEVICE_DIR}/${EXECUTABLE_NAME}" "${CURRENTCONFIG_SIMULATOR_DIR}/${EXECUTABLE_NAME}" || exit 1
	else
		echo "lipo: dir ${DEVICE}simulator not found: ${CURRENTCONFIG_SIMULATOR_DIR}"
	fi
else
	echo "lipo: dir ${DEVICE}os not found ${CURRENTCONFIG_DEVICE_DIR}"
fi
